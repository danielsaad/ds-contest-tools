...
def boca_pack(problem_folder: str, output_folder: str) -> None:
    ...
    info_log(
        f"Starting BOCA conversion for problem {os.path.basename(problem_folder)}.")
    ...
    # Get BOCA template and create BOCA folder
    if problem_metadata['problem']['interactive'] == False:
        boca_template_folder = os.path.join(
            *[os.path.dirname(os.path.abspath(__file__)), 'files', 'boca'])
    else:
        boca_template_folder = os.path.join(
            *[os.path.dirname(os.path.abspath(__file__)), 'files', 'boca', 'interactive'])
    ...
    input_files = [os.path.join(problem_input_folder, f) for
                   f in os.listdir(problem_input_folder) if os.path.isfile(os.path.join(problem_input_folder, f)) and not f.endswith('.interactive')]
